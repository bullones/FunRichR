...
```{r, eval=(EVAL_TF && !grepl("GSEA", LIST_LEVEL)), include=(EVAL_TF && !grepl("GSEA", LIST_LEVEL))}
...
for (i in 1:length(enrichment_list[[LIST_LEVEL]])) {
  ...
  if (grepl("OE", LIST_LEVEL)){
    tmp2 <- order(tmp$CountBg, decreasing = TRUE)[1:MAX_PLOT]
  ...
  if (grepl("GSEA", LIST_LEVEL)) {
    ...
    tmp2 <- c(head(tmp2, MAX_PLOT), tail(tmp, MAX_PLOT))
    tmp2 <- seq_len(nrow(tmp)) %in% tmp2
  }

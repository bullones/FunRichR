...
for (i in 1:length(enrichment_list[[LIST_LEVEL]])) {
  ...
  if (grepl("GSEA", LIST_LEVEL)){
    if (nrow(tmp2) < 2*MAX_PLOT) {
      MAX_PLOT <- floor(MAX_PLOT/3)
      message("MAX_PLOT value changed to ", MAX_PLOT, " for ", names(enrichment_list[[LIST_LEVEL]][i]))
    } # end if nrow(tmp)
  } # end if GSEA
...
```{r, fig.show = "hold", out.width = "50%", fig.height=ceiling(0.22*height_plots), fig.width=18, warning=FALSE, message=FALSE}
for (i in 1:length(enrichment_list[[LIST_LEVEL]])) {
  tmp <- enrichment_list[[LIST_LEVEL]][[i]]
  tmp2 <- tmp@result[tmp@result$p.adjust <= P_CUTOFF & tmp@result$qvalue <= Q_CUTOFF, ]
  # check if there are enough terms to be plotted

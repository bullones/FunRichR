...
```{r, eval=EVAL_TF && PROCESS, include=EVAL_TF && PROCESS}
# get TopGO
COUNTBG_CUTOFF <- 0.25
tmp <- data.frame()
...
```{r, eval=(EVAL_TF && ONT=="BP" && PROCESS), child='11_TF.Rmd'}
```

### Dotplot

```{r, fig.show = "hold", out.width = "50%", fig.height=0.12*ENRICH_FACTOR, fig.width=5, eval=PROCESS, include=PROCESS}
for (i in 1:length(enrichment_list[[LIST_LEVEL]])) {
  ...
  if (length(tmp4)){
    p_dp <- dotplot(enrichment_list[[LIST_LEVEL]][[i]],
                    ...
                    font.size = 6,
                    label_format = 80) + # sets wrap length
      ggtitle(paste0("Top (", round(COUNTBG_CUTOFF*100, 2), " %) GOs in ", names(enrichment_list[[LIST_LEVEL]][i]), " (", ONT, ")")) +
      theme(plot.title = element_text(hjust = 0.9))
  print(p_dp)
  } else {
...
```{r, warning=FALSE, eval=EVAL_TF && PROCESS, include=EVAL_TF && PROCESS}
# calculate heatmap
tmp_Count <- enrichment_list[[LIST_LEVEL]][[1]]@result[enrichment_list[[LIST_LEVEL]][[1]]@result$TopGO, c("Description", "Count")]
...
for (i in 2:length(enrichment_list[[LIST_LEVEL]])){
  tmp_Count <- merge(tmp_Count,
                     enrichment_list[[LIST_LEVEL]][[i]]@result[enrichment_list[[LIST_LEVEL]][[i]]@result$TopGO, c("Description", "Count")],
                     by = "Description",
                     all  = TRUE)
...
rownames(tmp_Count) <- tmp_Count$Description
tmp_Count$Description <- NULL
...
colnames(tmp_Count) <- names(enrichment_list[[LIST_LEVEL]])
colnames(tmp_CountBg) <- names(enrichment_list[[LIST_LEVEL]])
# tmp_Count <- t(t(tmp_Count)*c(1, -1))
# tmp_CountBg <- t(t(tmp_CountBg)*c(1, -1))
tmp_Count[, grepl("<", colnames(tmp_Count))] <- tmp_Count[, grepl("<", colnames(tmp_Count))] * -1
...
# rownames(tmp_CountBg) <- sapply(rownames(tmp_CountBg), function(x){
#   substr(x, 1, 90) # take the first 90 characters
# })
# rownames(tmp_CountBg) <- sub("^(.{90}).*", "\\1...", rownames(tmp_CountBg), perl = TRUE)
# rownames(tmp_CountBg) <- gsub("(.{90})", "\\1\n", rownames(tmp_CountBg), perl = TRUE)
...
```{r, fig.height=height, fig.width=width, warning=FALSE, eval=EVAL_TF && PROCESS, include=EVAL_TF && PROCESS}
...
```{r, eval=PROCESS, include=PROCESS}
...
```{r, fig.show = "hold", out.width = "50%", fig.height=0.15*ENRICH_FACTOR, fig.width=18, warning=FALSE, message=FALSE, eval=LIST_LEVEL=="BP-OE" && PROCESS, include=LIST_LEVEL=="BP-OE" && PROCESS}
...
```{r, fig.show = "hold", out.width = "50%", fig.height=6, fig.width=6, eval=PROCESS, include=PROCESS}
network_plots <- list()
...
```{r, echo=FALSE, fig.show = "hold", out.width = "50%", fig.height=6, fig.width=6, eval=PROCESS, include=PROCESS}
plt
```

### Wordclouds by words

```{bash, engine.opts='-l', results='hide', eval=PROCESS, include=PROCESS}
...
```{bash, engine.opts='-l', results='hide', eval=FALSE, include=FALSE, eval=PROCESS, include=PROCESS}
# Obtain stopwords
...
```{bash, message = FALSE, results='hide', eval=PROCESS, include=PROCESS}
...
```{bash, eval=PROCESS, include=PROCESS}
rm -r wordclouds
```

...
## Explicar características archivo a cargar

## Apuntes Enrique

FunRichR ENRIQUECIMIENTO FUNCIONAL:


Script de Amanda (git clone)
...
Venn, requiere la versión 1.11:
    > library(devtools)
    > install_version("venn", version = "1.11", repos = "http://cran.r-project.org")


Ejecutar el Script de configure_WGCNA.R, pero modificado:

allContrasts <- c("CJ.T16")
genes_list <- list()
tmp_up <- genes[genes[3] > 0,]
tmp_down <- genes[genes[3] < 0,]
genes_list[["CJ.T16_up"]] <- tmp_up[,1]
genes_list[["CJ.T16_down"]] <- tmp_down[,1]
